 ...
 * UI theo ảnh mẫu 4: danh sách phiếu bên trái, form nhập bên phải.
 * Nhập mã hàng (T20, M50...) → tự map. Chỉ Mã hàng, Số lượng, %CK được nhập.
 */
public class F1OpeningPanel extends BasePhieuPanel {
    ...
    private JPanel buildLeft() {
        ...
        p.setBackground(C_BG); p.setBorder(new EmptyBorder(8, 8, 8, 4));
        ...
        lblDateLeft.setFont(F_CODE_RED); lblDateLeft.setForeground(C_RED);
        ...
        btnFilter.addActionListener(e -> loadPhieuList());
        toolbar.add(prev); toolbar.add(lblDateLeft); toolbar.add(next); toolbar.add(btnFilter);

        // List header
        JPanel th = new JPanel(new GridLayout(1, 3));
        th.setBackground(C_TH_BG); th.setBorder(new EmptyBorder(4,6,4,6));
        for (String s : new String[]{"Số phiếu","Ngày nhập","Số tiền"}) {
            JLabel l = new JLabel(s, SwingConstants.CENTER); l.setFont(F_TH); l.setForeground(C_TH_FG); th.add(l);
        }

        listModel = new DefaultListModel<>();
        phieuList = new JList<>(listModel);
        phieuList.setFont(F_LIST); phieuList.setSelectionBackground(C_SEL_BILL); phieuList.setFixedCellHeight(26);
        ...
        JScrollPane scroll = new JScrollPane(phieuList);
        scroll.setBorder(BorderFactory.createLineBorder(C_BORDER));

        JPanel listArea = new JPanel(new BorderLayout());
        listArea.add(th, BorderLayout.NORTH); listArea.add(scroll, BorderLayout.CENTER);

        // Footer
        JPanel foot = new JPanel(new GridLayout(1,2));
        foot.setBackground(C_TOTAL_BG); foot.setBorder(BorderFactory.createMatteBorder(1,0,0,0,C_BORDER));
    ...
    private JPanel buildRight() {
        JPanel p = new JPanel(new BorderLayout(0,0));
        p.setBackground(C_BG); p.setBorder(new EmptyBorder(8,4,8,8));
        p.add(buildRightToolbar(), BorderLayout.NORTH);
        JPanel mid = new JPanel(new BorderLayout());
        mid.setBackground(C_WHITE); mid.setBorder(BorderFactory.createLineBorder(C_BORDER));
    ...
    private JPanel buildRightToolbar() {
        ...
        // date nav duplicate for right side
        ...
        btnFilter2.addActionListener(e -> loadPhieuList());
    ...
    private JPanel buildItemTable() {
        ...
        foot.setBackground(C_WHITE); foot.setBorder(BorderFactory.createMatteBorder(1,0,0,0,C_GRID));
        ...
        rf.add(lTTT); rf.add(lblTongThanhTien);

        JButton btnSave = mkBtn("💾 Lưu phiếu", C_GREEN, C_WHITE);
        btnSave.addActionListener(e -> savePhieu());
        rf.add(btnSave);
    ...
    private void resolveCode(int row) {
        Object o = itemModel.getValueAt(row,0);
        if (o==null) return;
        String code = o.toString().trim().toUpperCase();
        if (code.isEmpty()) { clearRow(row); return; }
        CardType ct = codeMap.get(code);
        if (ct==null) { clearRow(row); return; }
        ...
        final int fqty=qty, fgiaCK=giaCK;
        suppress(itemModel,()->{
            itemModel.setValueAt(ct.name,row,1); itemModel.setValueAt("Thẻ",row,2);
            itemModel.setValueAt(ct.denomination,row,4); itemModel.setValueAt(ckPct,row,5);
            itemModel.setValueAt(fqty,row,3); itemModel.setValueAt((long)fqty*fgiaCK,row,6);
        });
        refreshTotals();
    }

    private void clearRow(int row) {
        suppress(itemModel,()->{ itemModel.setValueAt("",row,1); itemModel.setValueAt("Thẻ",row,2); itemModel.setValueAt(0,row,3); itemModel.setValueAt(0,row,4); itemModel.setValueAt(0.0,row,5); itemModel.setValueAt(0,row,6); });
        refreshTotals();
    }

    private void recalcRow(int row) {
        int denom=toInt(itemModel.getValueAt(row,4)); if(denom==0)return;
        int qty=toInt(itemModel.getValueAt(row,3));
        double ck=toDbl(itemModel.getValueAt(row,5));
        int giaCK=(int)Math.round(denom*(1.0-ck/100.0));
        suppress(itemModel,()->itemModel.setValueAt((long)qty*giaCK,row,6));
        refreshTotals();
    }

    private void refreshTotals() {
        long t=0; for(int i=0;i<itemModel.getRowCount();i++) t+=toLong(itemModel.getValueAt(i,6));
    ...
    private void newPhieu() {
        ...
        if(phieuList!=null) phieuList.clearSelection();
        refreshTotals();
    }

    private void loadPhieuList() {
        if(listModel==null) return;
        listModel.clear(); long total=0;
    ...
    private void loadPhieuIntoForm(PhieuEntry entry) {
        ...
        try(Connection conn=DBConnection.getConnection(); PreparedStatement ps=conn.prepareStatement(
            ...
            while(rs.next()){
                ...
                itemModel.addRow(new Object[]{ct2!=null?ct2.code:"",rs.getString("name"),"Thẻ",qty,denom,ck,(long)qty*ckPri});
            }
        } catch(Exception ex){ex.printStackTrace();}
        while(itemModel.getRowCount()<12) addEmptyRow();
        refreshTotals();
    }
    ...
    private void savePhieu() {
        ...
        for(int i=0;i<itemModel.getRowCount();i++){
            String code=itemModel.getValueAt(i,0).toString().trim().toUpperCase();
    ...
    private void deletePhieu() {
        PhieuEntry sel=phieuList.getSelectedValue();
        if(sel==null){showError("Chọn phiếu cần xóa!");return;}
    ...
    private class PhieuCellRenderer extends DefaultListCellRenderer {
        @Override public Component getListCellRendererComponent(JList<?> list,Object val,int idx,boolean sel,boolean foc){
            PhieuEntry e=(PhieuEntry)val;
            JPanel p=new JPanel(new GridLayout(1,3)); p.setOpaque(true);
