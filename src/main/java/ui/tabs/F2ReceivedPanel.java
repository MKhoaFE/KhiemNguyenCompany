...
/**
 * F2 - Nhập Hàng — Phiếu nhập hàng
 * - Bỏ cột ĐVT
 * - 1 hàng mặc định; gõ mã hàng → tự sinh hàng mới
 * - Nút "In phiếu" → HTML preview (InvoiceBuilder), có tên NCC
 */
public class F2ReceivedPanel extends BasePhieuPanel {
    ...
    private JPanel buildFormHeader() {
        ...
        p.add(lbl("Số phiếu"));p.add(lblSoPhieu);p.add(lbl("  Ngày nhập"));p.add(txtNgay);
        p.add(chkTT);p.add(rCK);p.add(rGia);p.add(lbl("  NCC"));p.add(txtNCC);p.add(lbl("  Ghi chú"));p.add(txtGhiChu);
        return p;
    }
    ...
    private JPanel buildItemTable() {
        ...
        itemTable.setDefaultRenderer(Object.class,new F2Renderer());
    ...
    private void loadPhieuIntoForm(PhieuEntry entry){
        ...
        if(entry.ncc!=null)txtNCC.setText(entry.ncc);
        ...
        try(Connection conn=DBConnection.getConnection();PreparedStatement ps=conn.prepareStatement(
            ...
            while(rs.next()){int qty=rs.getInt("quantity"),denom=rs.getInt("denomination"),ckPri=rs.getInt("discount_price");
    ...
    private void savePhieu(){
        ...
        for(int i=0;i<itemModel.getRowCount();i++){
            ...
            double ck=toDbl(itemModel.getValueAt(i,COL_CK));int ckPrice=(int)Math.round(ct.denomination*(1.0-ck/100.0));
    ...
    private void shiftDate(int d){
        currentDate=currentDate.plusDays(d);
        if(lblDateLeft!=null)lblDateLeft.setText(currentDate.format(DATE_FMT));
        if(txtNgay!=null)txtNgay.setText(currentDate.format(DATE_FMT));
        loadPhieuList();
    }
    ...
    private void showPrintPreview(){
        ...
        String ncc=txtNCC!=null?txtNCC.getText():"";
        String ghiChu=txtGhiChu!=null?txtGhiChu.getText():"";

        List<InvoiceBuilder.ReceiptItem> items=new ArrayList<>();
        for(int i=0;i<itemModel.getRowCount();i++){
            String code=itemModel.getValueAt(i,COL_CODE).toString().trim();if(code.isEmpty())continue;
            int qty=toInt(itemModel.getValueAt(i,COL_QTY));if(qty<=0)continue;
            InvoiceBuilder.ReceiptItem it=new InvoiceBuilder.ReceiptItem();
            it.name=itemModel.getValueAt(i,COL_NAME).toString();
            it.quantity=qty;it.unitPrice=toInt(itemModel.getValueAt(i,COL_PRICE));
            it.ckPercent=toDbl(itemModel.getValueAt(i,COL_CK));it.lineTotal=toLong(itemModel.getValueAt(i,COL_TOTAL));
            items.add(it);
        }
        String html=InvoiceBuilder.buildReceiptInvoice(soPhieu,ngay,ncc,ghiChu,items);
        PrintPreviewDialog.show(this,"Phiếu nhập "+soPhieu,html);
    }

    // ═══ DATA + RENDERERS ═════════════════════════════════════════════════════

    private static class PhieuEntry{
        int id;LocalDate date;String soPhieu,ncc;long total;
        PhieuEntry(int id,LocalDate d,String sp,String ncc,long t){this.id=id;date=d;soPhieu=sp;this.ncc=ncc;total=t;}
    }
    ...
    private class F2Renderer extends DefaultTableCellRenderer{
